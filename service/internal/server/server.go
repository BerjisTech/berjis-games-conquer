...
func New(opts Options) *fiber.App {
    ...
    isAdmin := func(uid string) bool {
        if opts.AdminUsers == "" { return false }
        for _, p := range strings.Split(opts.AdminUsers, ",") {
            if strings.TrimSpace(p) == uid { return true }
        }
        return false
    }
...
ON CONFLICT (code) DO NOTHING;`)
    ...
    app.Get("/v1/countries/geo", func(c *fiber.Ctx) error {
        if opts.DB == nil { return c.JSON(fiber.Map{"success": true, "data": fiber.Map{"type":"FeatureCollection","features": []any{}}}) }
        type row struct { ID string `db:"id"`; Name string `db:"name"`; Geo json.RawMessage `db:"geo"` }
        rows := []row{}
        ...
        for _, r := range rows {
            ...
            if err := json.Unmarshal(r.Geo, &tmp); err == nil {
                if t, _ := tmp["type"].(string); t == "Feature" {
                    if props, ok := tmp["properties"].(map[string]any); ok { props["id"] = r.ID; props["name"] = r.Name } else { tmp["properties"] = map[string]any{"id": r.ID, "name": r.Name} }
    ...
    app.Post("/v1/players/join", func(c *fiber.Ctx) error {
        ...
        uid, err := getUserID(c); if err != nil { return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{"success": false}) }
        var in struct{ CountryID string `json:"countryId"` }
    ...
    app.Post("/v1/players/desert", func(c *fiber.Ctx) error {
        ...
        type prof struct { CountryID *string `db:"country_id"`; LastDesertAt *time.Time `db:"last_desert_at"` }
        var p prof
    ...
    app.Get("/v1/players/me/loot", func(c *fiber.Ctx) error {
        if opts.DB == nil { return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false}) }
        uid, err := getUserID(c); if err != nil { return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{"success": false}) }
        if err := ensureLoot(opts.DB, "player", uid); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
    ...
    app.Post("/v1/loot/contribute", func(c *fiber.Ctx) error {
        if opts.DB == nil { return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false}) }
        uid, err := getUserID(c); if err != nil { return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{"success": false}) }
        var in struct{ Resources map[string]int64 `json:"resources"` }
        ...
        if err := ensureLoot(opts.DB, "player", uid); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        if err := ensureLoot(opts.DB, "country", countryID); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        // Load current player resources
        var cur map[string]int64
        if err := getResources(opts.DB, "player", uid, &cur); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        // Check and subtract
        for k, v := range in.Resources {
            if v < 0 { return c.Status(400).JSON(fiber.Map{"success": false, "message": "invalid amount"}) }
            if cur[k] < v { return c.Status(409).JSON(fiber.Map{"success": false, "message": "insufficient resources"}) }
            cur[k] -= v
        }
        if err := setResources(opts.DB, "player", uid, cur); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        // Add to country loot
        var base map[string]int64
        _ = getResources(opts.DB, "country", countryID, &base)
        for k, v := range in.Resources { base[k] += v }
        if err := setResources(opts.DB, "country", countryID, base); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        return c.JSON(fiber.Map{"success": true})
    })
    ...
    app.Post("/v1/admin/seed/resources", func(c *fiber.Ctx) error {
        ...
        uid, err := getUserID(c); if err != nil { return c.Status(401).JSON(fiber.Map{"success": false}) }
        if !isAdmin(uid) { return c.Status(403).JSON(fiber.Map{"success": false}) }
        var in struct{ UserID string `json:"userId"`; Resources map[string]int64 `json:"resources"` }
        if err := c.BodyParser(&in); err != nil || len(in.Resources) == 0 { return c.Status(400).JSON(fiber.Map{"success": false}) }
        target := in.UserID; if target == "" { target = uid }
        if err := ensureLoot(opts.DB, "player", target); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        var cur map[string]int64
        if err := getResources(opts.DB, "player", target, &cur); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        for k, v := range in.Resources { cur[k] += v }
        if err := setResources(opts.DB, "player", target, cur); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        return c.JSON(fiber.Map{"success": true})
    })
    ...
    app.Get("/v1/admin/seed/resources", func(c *fiber.Ctx) error {
        if !opts.AdminGetSeed { return c.Status(405).JSON(fiber.Map{"success": false, "message": "GET not enabled"}) }
        if opts.DB == nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        uid, err := getUserID(c); if err != nil { return c.Status(401).JSON(fiber.Map{"success": false}) }
        if !isAdmin(uid) { return c.Status(403).JSON(fiber.Map{"success": false}) }
        ...
        for _, k := range []string{"metal","fuel","intel","jeep","tank","radar"} {
            if v := c.Query(k); v != "" {
                if n, err := strconv.ParseInt(v, 10, 64); err == nil { res[k] = n }
            }
        }
        if len(res) == 0 { return c.Status(400).JSON(fiber.Map{"success": false, "message": "no resources"}) }
        if err := ensureLoot(opts.DB, "player", target); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        var cur map[string]int64
        if err := getResources(opts.DB, "player", target, &cur); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        for k, v := range res { cur[k] += v }
        if err := setResources(opts.DB, "player", target, cur); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        return c.JSON(fiber.Map{"success": true})
    })
    ...
    app.Get("/admin/seed", func(c *fiber.Ctx) error {
        if !opts.AdminGetSeed { return c.Status(404).SendString("not found") }
...
<body>
  ...
  <form method="post" action="/v1/admin/seed/resources">
    ...
    app.Post("/v1/craft/orders", func(c *fiber.Ctx) error {
        if opts.DB == nil { return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false}) }
        uid, err := getUserID(c); if err != nil { return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{"success": false}) }
        var in struct{ RecipeID string `json:"recipeId"` }
        ...
        if cost == nil { return c.Status(400).JSON(fiber.Map{"success": false, "message": "unknown recipe"}) }
        if err := ensureLoot(opts.DB, "player", uid); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        var cur map[string]int64
        if err := getResources(opts.DB, "player", uid, &cur); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        for k, v := range cost {
