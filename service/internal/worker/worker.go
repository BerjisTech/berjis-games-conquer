...
func processDueOrders(db *sqlx.DB, logger *log.Logger) error {
	...
	for _, o := range rows {
		...
		if err := ensureLoot(tx, "player", o.PlayerID); err != nil {
			tx.Rollback()
			return err
		}
		var m map[string]int64
		if err := getResources(tx, "player", o.PlayerID, &m); err != nil {
			tx.Rollback()
			return err
		}
		m[o.RecipeID] = m[o.RecipeID] + 1
		if err := setResources(tx, "player", o.PlayerID, m); err != nil {
			tx.Rollback()
			return err
		}
